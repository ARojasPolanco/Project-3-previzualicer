...
function App() {
  ...
  return (
    <main className="bg-[url(/images/bg-fscreen.png)] min-h-screen h-full w-full flex flex-col items-center font-Fira">
      ...
      <section className="h-[400px] w-10/12 flex justify-center mb-6 -mt-6 relative min-[400px]:w-[280px]">
        <section className={`absolute w-full ${showCards === idCards.id1 ? 'top-0 opacity-1 visible' : '-top-96 opacity-0 invisible'} transition-[opacity_visibility] duration-1000`}>
          <CharactersCard image={"/images/porci.png"} name={'Martin Porciúncula'} country={'Uruguay'} url={idUrl.id1} />
        </section>
        <section className={`absolute w-full ${showCards === idCards.id2 ? 'top-0 opacity-1 visible' : '-top-96 opacity-0 invisible'} transition-[opacity_visibility] duration-1000`}>
          {<CharactersCard image={"/images/rick-morty-robot.png"} name={'Edwar León'} country={'Colombia'} url={idUrl.id2} />}</section>
        <section className={`absolute w-full ${showCards === idCards.id3 ? 'top-0 opacity-1 visible' : '-top-96 opacity-0 invisible'} transition-[opacity_visibility] duration-1000`}>
